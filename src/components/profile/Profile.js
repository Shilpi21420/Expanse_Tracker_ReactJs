...
export default function Profile() {
  ...
  useEffect(() => {
    async function myfun() {
      const response = await axios.post(
        "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyCvrnESoYx47P4lBHOYNCfpLfno9bGzfGY",
        token
      );
  ...
  const updatedetails = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyCvrnESoYx47P4lBHOYNCfpLfno9bGzfGY",
        details
      );
  ...
  const verify=async(event)=>{
    ...
    const details = {
     ...
     try {
        const response = await axios.post(
          "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyCvrnESoYx47P4lBHOYNCfpLfno9bGzfGY",
          details
        );
